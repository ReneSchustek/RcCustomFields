...
// der Plugin-Code löst beide als freie Bezeichner gegen diese Locals auf.
const wrapped = `
...
// Minimales Form mit Event-Buchhaltung; kein dataset -> _collectAllSuffixes iteriert über nichts.
function fakeForm() {
    const dispatched = [];
    return {
        dispatched,
...
describe('ID-Hoheit hängt an der Befüllung, nicht am Plugin', () => {
    ...
    test('befülltes Feld beansprucht Hoheit über Marker und DOM-Attribut', () => {
    ...
    test('befülltes Feld berechnet eine von der Produkt-ID abweichende Positions-ID', () => {
        const instance = makeInstance({ inputs: [textInput('Hans Muster')] });
...
describe('Suffix-Event nur beim Wechsel der Hoheit', () => {
    test('unverändert leere Felder feuern auch bei wiederholtem Update kein Event', () => {
        const instance = makeInstance({ inputs: [textInput('')] });
    ...
    test('Leeren eines zuvor befüllten Feldes feuert genau ein Suffix-Event mit eigener Quelle', () => {
    ...
    test('Leeren gibt die ID zurück auf die Produkt-ID und räumt den Marker', () => {
        const input = textInput('Hans Muster');
        const instance = makeInstance({ inputs: [input] });
        instance._updateLineItemId();
...
describe('stackSameValues=0 uebt Hoheit unabhängig von Eingaben aus', () => {
